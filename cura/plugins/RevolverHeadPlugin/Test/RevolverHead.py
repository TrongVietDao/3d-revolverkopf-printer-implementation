...
class RevolverHead(QObject, Extension):
    def __init__(self, parent = None) -> None:
        QObject.__init__(self, parent)
        Extension.__init__(self)
        self.setMenuName(i18n_catalog.i18nc("@item:inmenu", "Test Extension"))
        self.addMenuItem(i18n_catalog.i18nc("@item:inmenu", "Do Test Stuff"), self.showPopup)
        self._view = None
        ...
        def moveScript(self, index: int, new_index: int) -> None:
            if new_index < 0 or new_index > len(self._script_list) - 1:
                ...
                # Magical switch code.
                self._script_list[new_index], self._script_list[index] = self._script_list[index], self._script_list[
                    new_index]
        ...
        def loadAllScripts(self) -> None:
            ...
            for root in [PluginRegistry.getInstance().getPluginPath("RevolverHeadPlugin"),
                ...
                if script_name not in sys.modules:
                    try:
                        spec = importlib.util.spec_from_file_location(__name__ + "." + script_name,
                                                                      os.path.join(path, script_name + ".py"))
                        ...
                        try:
                            ...
                            Logger.log("e", "Script %s.py is not a recognised script type. Ensure it inherits Script",
                                       script_name)
                        except NotImplementedError:
                            Logger.log("e", "Script %s.py has no implemented settings", script_name)
                    except Exception as e:
                        Logger.logException("e",
                                            "Exception occurred while loading post processing plugin: {error_msg}".format(
                                                error_msg=str(e)))
        ...
        def writeScriptsToStack(self) -> None:
            ...
            for script in self._script_list:
                parser = configparser.ConfigParser(
                    interpolation=None)  # We'll encode the script as a config with one section. The section header is the key and its values are the settings.
            ...
            script_list_string = "\n".join(
                script_list_strs)  # ConfigParser should never output three newlines in a row when serialised, so it's a safe delimiter.
        ...
        def _createView(self) -> None:
            ...
            path = os.path.join(cast(str, PluginRegistry.getInstance().getPluginPath("RevolverHeadPlugin")),
                                "RevolverHeadPlugin.qml")
            self._view = CuraApplication.getInstance().createQmlComponent(path, {"manager": self})
            if self._view is None:
                Logger.log("e",
                           "Not creating PostProcessing button near save button because the QML component failed to be created.")
                return
            Logger.log("d", "Post processing view created.")

            # Create the save button component
            CuraApplication.getInstance().addAdditionalComponent("saveButton", self._view.findChild(QObject,
