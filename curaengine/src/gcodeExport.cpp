...
void GCodeExport::writeServoStartingPosition()
{
    *output_stream << "M280 P0 S0" << new_line;
}
...
int GCodeExport::determineServoPosByExtruderNr(size_t extruder)
{
    switch(extruder)
    {
        case 0:
...
int GCodeExport::switchExtruderNozzle(size_t new_extruder, const RetractionConfig& retraction_config_old_extruder, coord_t perform_z_hop)
{
    ...
    const int servo_pos = determineServoPosByExtruderNr(new_extruder);

    assert(servo_pos >= 0);
    *output_stream << "M280 P0 S" << servo_pos << new_line;
