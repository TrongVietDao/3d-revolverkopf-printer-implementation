...
public:
    ...
    /*!
     * Write servo 0 to position 0.
     */
    void writeServoStartingPosition();
private:
    /*!
     * Coordinates are build plate coordinates, which might be offsetted when extruder offsets are encoded in the gcode.
     * 
...
public:
    ...
    /*!
    * Get the servo position for the given extruder.
    * @param Extruder number to get servo position of.
    * @return Servo position for the requested extruder.
    */
    int determineServoPosByExtruderNr(size_t extruder)
     ...
     */
    int switchExtruderNozzle(size_t new_extruder, const RetractionConfig& retraction_config_old_extruder, coord_t perform_z_hop = 0);
