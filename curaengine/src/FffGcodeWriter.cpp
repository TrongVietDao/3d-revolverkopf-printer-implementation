...
void FffGcodeWriter::writeGCode(SliceDataStorage& storage, TimeKeeper& time_keeper)
{
    ...
    gcode.writeServoStartingPosition();
...
#pragma omp critical
    ...
    LayerPlan& next_gcode_layer = *new LayerPlan(storage, layer_nr + 1, z, layer_thickness, extruder_order.front(), fan_speed_layer_time_settings_per_extruder, comb_offset_from_outlines, first_outer_wall_line_width, avoid_distance);
...
void FffGcodeWriter::addCustomMeshLayerToGCode(const SliceDataStorage& storage, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, LayerPlan& gcode_layer, const size_t total_layers) const
{
    if (gcode_layer.getLayerNr() > mesh.layer_nr_max_filled_layer)
    {
        return;
    ...
    if (mesh.isPrinted())
    {
        ...
        {
            const SliceLayerPart& part = layer.parts[part_idx];
            addCustomMeshPartToGCode(storage, mesh, extruder_nr, mesh_config, part, gcode_layer);
        }
    }
    else
    {
        ...
        while (parts.size())
        {
            ...
            addCustomMeshPartToGCode(storage, mesh, extruder_nr, mesh_config, *parts[nearest_part_index], gcode_layer);
...
void FffGcodeWriter::addCustomMeshPartToGCode(const SliceDataStorage& storage, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SliceLayerPart& part, LayerPlan& gcode_layer, const size_t total_layers) const
{
    const Settings& mesh_group_settings = Application::getInstance().current_slice->scene.current_mesh_group->settings;
    ...
    const ExtruderTrain& train = Application::getInstance().current_slice->scene.extruders[extruder_nr]
    const double nozzle_size = train.settings.get<double>("machine_nozzle_size")
    coord_t layer_thickness = mesh_group_settings.get<coord_t>("layer_height");

    assert(gcode_layer.getLayerNr() % ceil(nozzle_size / layer_thickness) == 0);
    if ((gcode_layer.getLayerNr() % ceil(nozzle_size / layer_thickness) == 0 || gcode_layer.getLayerNr() == total_layers) && extruder_nr != mesh.settings.get<ExtruderTrain&>("infill_extruder_nr").extruder_nr)
    {
        if (mesh.settings.get<bool>("infill_before_walls"))
        {
            added_something = added_something | processCustomInfill(storage, gcode_layer, mesh, extruder_nr, mesh_config, part);
        }

        added_something = added_something | processCustomInsets(storage, gcode_layer, mesh, extruder_nr, mesh_config, part);
...
bool FffGcodeWriter::processCustomInfill(const SliceDataStorage& storage, LayerPlan& gcode_layer, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SliceLayerPart& part) const
{
    if (extruder_nr != mesh.settings.get<ExtruderTrain&>("infill_extruder_nr").extruder_nr)
    {
        return false;
    }
    const ExtruderTrain& extruderTrain = mesh.settings.get<ExtruderTrain&>("infill_extruder_nr");

    if (extruderTrain.settings.getServoPos() == gcode.determineServoPosByExtruderNr(extruder_nr))
    {
        const RetractionConfig retractionConfig = storage.extruder_switch_retraction_config_per_extruder[extruder_nr];
        const coord_t zhop = extruderTrain.settings.get<coord_t>("retraction_hop_after_extruder_switch_height");
        const int newServoPos = gcode.switchExtruderNozzle(extruder_nr, retractionConfig, zhop);
        assert(newServoPos >= 0);
        extruderTrain.settings.setServoPosition(newServoPos);
    }
...
bool FffGcodeWriter::processCustomInsets(const SliceDataStorage& storage, LayerPlan& gcode_layer, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SliceLayerPart& part) const
{
    ...
    if (extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_0_extruder_nr").extruder_nr) {
        extruderTrain = mesh.settings.get<ExtruderTrain&>("wall_0_extruder_nr");
    } else if (extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_x_extruder_nr").extruder_nr) {
        extruderTrain = mesh.settings.get<ExtruderTrain&>("wall_x_extruder_nr");
    }
        ...
        else
        {
            ...
            for (int inset_number = part.insets.size() - 1; inset_number > -1; inset_number--)
            {
                ...
                else if (!part.insets[processed_inset_number].empty() && extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_x_extruder_nr").extruder_nr)
                {
                    ...
                    if (gcode_layer.getLayerNr() % ceil(nozzle_size / layer_thickness) == 0 || gcode_layer.getLayerNr() == total_layers) {
                        setExtruder_addPrime(storage, gcode_layer, extruder_nr);
                        gcode_layer.setIsInside(true); // going to print stuff inside print object
                        ZSeamConfig z_seam_config(mesh.settings.get<EZSeamType>("z_seam_type"), mesh.getZSeamHint(), mesh.settings.get<EZSeamCornerPrefType>("z_seam_corner"));
